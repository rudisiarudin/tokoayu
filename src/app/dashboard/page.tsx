...
import { debts, products, stockHistory, transactions } from "@/lib/dummy-data";
import { rupiah } from "@/lib/utils";
import { cn } from "@/lib/utils";

const lowStock = products.filter((item) => item.stock <= item.minStock);
const todaySales = 2450000;
const todayProfit = 612500;
const unpaidDebt = 3850000;
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard title="Penjualan Hari Ini" value={rupiah(todaySales)} note="+ 12% dari kemarin" tone="green" />
        <MetricCard title="Perkiraan Untung" value={rupiah(todayProfit)} note="+ 8% dari kemarin" tone="green" />
        <MetricCard title="Hutang Belum Lunas" value={rupiah(unpaidDebt)} note={`${debts.length + 21} pelanggan`} tone="orange" />
        <MetricCard title="Stok Hampir Habis" value={`${lowStock.length + 8} Barang`} note="Perlu restock" tone="red" />
      </div>
              ...
              return (
                <div key={product.id} className={cn(
                  ...
                  <div className="hidden md:block text-center">
                    ...
                    <p className="text-xs font-extrabold text-foreground">{product.minStock} {product.unit}</p>
                  </div>
            ...
            <div className="p-5 flex flex-col gap-3">
              {transactions.slice(0, 4).map((trx) => (
                <div key={trx.id} className="flex items-center justify-between gap-3 rounded-2xl border border-[#e8ece9] bg-white p-3.5 hover:border-emerald-500/25 transition-all">
                  ...
                  <div>
                    <Button variant="ghost" className="h-8 rounded-xl text-[10px] font-extrabold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-3">
                      Detail
                    </Button>
                  </div>
                </div>
...
function MetricCard({
  ...
  return (
    <Card className={`rounded-[22px] border transition-all duration-300 hover:shadow-md hover:scale-[1.01] ${style[tone]}`}>
